...
export default function PartnersPage() {
  ...
  const handleAcceptPact = async (invite: PactInvite) => {
    const res = await fetch(`http://localhost:3001/api/pacts/${invite.id}/accept`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${token}` },
    });
    if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Accountability Partners</h1>

        {/* Search + Invite */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
          <h2 className="font-semibold text-gray-700 mb-3">Find a partner by username</h2>
          <input
            type="text"
          ...
          {searchResults.length > 0 && (
            <ul className="mt-3 divide-y divide-gray-100 border border-gray-200 rounded-lg overflow-hidden">
              {searchResults.map((u) => (
                <li key={u.id} className="flex items-center justify-between px-4 py-3 bg-white">
                  <Link to={`/profile/${u.username}`} className="text-sm font-medium text-gray-800 hover:text-indigo-600 hover:underline">
                    @{u.username}
                  </Link>
                  <button
        ...
        {pactInvites.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-purple-200 p-5 mb-6">
            <h2 className="font-semibold text-purple-700 mb-3 flex items-center gap-2">
              <span className="text-purple-500"><PactIcon /></span>
              Pact Invites
            </h2>
              ...
              {pactInvites.map((invite) => (
                <li key={invite.id} className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">"{invite.habit.name}"</p>
                  </div>
        ...
        {pending.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-yellow-200 p-5 mb-6">
            <h2 className="font-semibold text-yellow-700 mb-3">Pending Invites</h2>
            <ul className="divide-y divide-gray-100">
              {pending.map((p) => (
                <li key={p.id} className="flex items-center justify-between py-3">
                  <span className="text-sm font-medium text-gray-800">
                    <Link to={`/profile/${getOtherUser(p).username}`} className="hover:text-indigo-600 hover:underline">
                      @{getOtherUser(p).username}
                    </Link>{" "}wants to be your partner
                  </span>
                  <div className="flex gap-2">
                    <button onClick={() => handleAccept(p.id)} className="text-sm bg-green-600 text-white px-3 py-1 rounded-lg hover:bg-green-700">Accept</button>
                    <button onClick={() => handleRevoke(p.id)} className="text-sm bg-red-100 text-red-600 px-3 py-1 rounded-lg hover:bg-red-200">Ignore</button>
                  </div>
                </li>
              ))}
            </ul>
            ...
            <ul className="divide-y divide-gray-100">
              {sentPending.map((p) => (
                <li key={p.id} className="flex items-center justify-between py-3">
                  ...
                  <button onClick={() => handleRevoke(p.id)} className="text-sm text-red-500 hover:underline">Cancel</button>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
          ...
          {isLoading ? (
            ...
            <p className="text-sm text-gray-400">No partners yet. Invite someone above!</p>
          ) : (
            <ul className="divide-y divide-gray-100">
              {accepted.map((p) => (
                <li key={p.id} className="flex items-center justify-between py-3">
                  <Link to={`/profile/${getOtherUser(p).username}`} className="text-sm font-medium text-gray-800 hover:text-indigo-600 hover:underline">
                    @{getOtherUser(p).username}
                  </Link>
                  <button onClick={() => setPartnerToRemove(p)} className="text-sm text-red-500 hover:underline">Remove</button>
                </li>
              ))}
            </ul>
          )}
